...
(`docs/poster.jpg`, generated by `python -m winduo.tools.make_og --poster`, or by
grabbing a frame from the real clip once one exists) stands in, so the slot never
renders as a broken element.

Keep both video files small; this loads on every visit to the page, not on demand.
Re-encoding the current clip to roughly 640px wide, 30fps, muted, gets it under 2MB
for each format:

```sh
ffmpeg -i demo_source.mp4 -vf "scale=640:-2,fps=30" -an -c:v libx264 -crf 28 \
  -preset medium -profile:v main -pix_fmt yuv420p -movflags +faststart docs/demo.mp4
ffmpeg -i docs/demo.mp4 -c:v libvpx-vp9 -b:v 0 -crf 34 -an docs/demo.webm
ffmpeg -i docs/demo.mp4 -vf "select=eq(n\,30)" -vframes 1 -q:v 4 docs/poster.jpg
```
